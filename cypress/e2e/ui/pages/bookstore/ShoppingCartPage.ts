...
class ShoppingCartPage extends BasePage {
  ...
  protected shoppingCartElements = {
    shoppingCart: () =>
      cy.get(`${this.shoppingCartSelectors.shoppingCartHeader}`),
    emptyCart: () => cy.get(`${this.shoppingCartSelectors.emptyCartHeader}`),
    quantity: () => cy.get(`${this.shoppingCartSelectors.quantityInput}`),
    updateButton: () =>
      cy
        .get(`${this.shoppingCartSelectors.quantityInput}`)
        .parent()
        .find(`${this.shoppingCartSelectors.updateButton}`),
    deleteButton: () => cy.get(`${this.shoppingCartSelectors.deleteLink}`),
    checkoutButton: () =>
      cy.get(`${this.shoppingCartSelectors.checkoutButton}`),
    totalAmount: () =>
      cy
        .get(`${this.shoppingCartSelectors.checkoutButton}`)
        .parent()
        .find(`${this.shoppingCartSelectors.span}`),
    price: () =>
      cy
        .get(`${this.shoppingCartSelectors.tableBody}`)
  ...
  verifyCartIsCleaned() {
    this.elements
      ...
      .getSelector()
      .then((selector) => {
        cy.percySnapshot("Shopping Cart Page - Empty Cart", {
          scope: selector,
        });
      });
  }
    ...
    return this.shoppingCartElements
      ...
      .then(($rows) => {
        ...
        $rows.each((_, row) => {
          ...
          const quantityElement = $row.find(
            this.shoppingCartSelectors.quantityInput
          );
          ...
          if (quantityElement.length && priceElement.length) {
            ...
            const { price, currency } = this.parsePrice(
  ...
  private computeFinalTotal(
    calculations: Array<{ quantity: number; price: number; currency: string }>
  ): string {
    ...
    const detectedCurrency =
      calculations.find((calc) => calc.currency)?.currency || "";
