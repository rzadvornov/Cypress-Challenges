...
class SignInPage extends BasePage {
  ...
  verifyPageUrl() {
    cy.url().should('eq', `${Cypress.config().baseUrl}${Cypress.env('bookstore_url')}${Cypress.env('signIn_url')}`);
  }
  ...
  verifyPageLoaded(): void {
    ...
    this.signInElements.signIn()
      .should('be.visible')
      .contains('Sign in');
    this.signInElements.signInButton()
      .should('be.visible')
      .contains('Sign In');
    this.signInElements.signUpButton()
      .should('be.visible')
      .contains('Sign Up!');
  }
