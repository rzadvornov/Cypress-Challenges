...
class DragAndDropPage extends BasePage {
  ...
  protected dragDropElements = {
    ...
    column: (element: string) =>
      cy.get(`${this.dragDropSelectors.columnPrefix}${element.toLowerCase()}`),
    columnHeader: (element: string) =>
      cy
        .get(`${this.dragDropSelectors.columnPrefix}${element.toLowerCase()}`)
        .find(`${this.dragDropSelectors.headerTag}`),
    dndColumns: () =>
      cy
        .get(`${this.dragDropSelectors.dndColumnsContainer}`)
        .find(`${this.dragDropSelectors.columnClass}`),
    columnByIndex: (index: number) =>
      cy
        .get(`${this.dragDropSelectors.dndColumnsContainer}`)
        .find(`${this.dragDropSelectors.columnClass}`)
        .eq(index),
  ...
  verifyElement(element: string) {
    this.dragDropElements.column(element).should("be.visible");

    this.dragDropElements.columnHeader(element).should("contain.text", element);
  }

  verifyPageUrl() {
    cy.url().should(
      "eq",
      `${Cypress.config().baseUrl}${Cypress.env("drag_and_drop_url")}`
    );
  }
  ...
  performDragAndDrop(elementA: string, elementB: string) {
    this.dragDropElements
      .column(elementA)
      .drag(`#column-${elementB.toLowerCase()}`);
  }

  verifyDragAndDrop(element: string, column: string) {
    this.dragDropElements
      .columnByIndex(parseInt(column, 10))
      .find("header")
