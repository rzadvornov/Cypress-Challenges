...
class FormValidationPage extends BasePage {
  private readonly validationSelectors = {
    ...
    contactNameContainer:
      'form[action="/form-confirmation"] > div:has(> #validationCustom01)',
    contactNumberContainer:
      'form[action="/form-confirmation"] > div:has(> #validationCustom05[name="contactnumber"])',
    dateContainer:
      'form[action="/form-confirmation"] > div:has(> #validationCustom05[name="pickupdate"])',
    paymentMethodContainer:
      'form[action="/form-confirmation"] > div:has(> #validationCustom04)',
  } as const;

  protected validationElements = {
    contactNameField: () =>
      cy.get(`${this.validationSelectors.contactNameField}`),
    contactNumberField: () =>
      cy.get(`${this.validationSelectors.contactNumberField}`),
    pickupDateField: () =>
      cy.get(`${this.validationSelectors.pickupDateField}`),
    paymentMethodField: () =>
      cy.get(`${this.validationSelectors.paymentMethodField}`),
    submitButton: () =>
      cy
        .get(`${this.validationSelectors.formAction}`)
        .find(`${this.validationSelectors.submitButton}`),
    // Validation feedback elements
    contactNameInvalidFeedback: () =>
      cy
        .get(`${this.validationSelectors.contactNameContainer}`)
        .find(`${this.validationSelectors.invalidFeedback}`),
    contactNameValidFeedback: () =>
      cy
        .get(`${this.validationSelectors.contactNameContainer}`)
        .find(`${this.validationSelectors.validFeedback}`),
    contactNumberInvalidFeedback: () =>
      cy
        .get(`${this.validationSelectors.contactNumberContainer}`)
        .find(`${this.validationSelectors.invalidFeedback}`),
    dateInvalidFeedback: () =>
      cy
        .get(`${this.validationSelectors.dateContainer}`)
        .find(`${this.validationSelectors.invalidFeedback}`),
    paymentMethodInvalidFeedback: () =>
      cy
        .get(`${this.validationSelectors.paymentMethodContainer}`)
        .find(`${this.validationSelectors.invalidFeedback}`),
  };
  ...
  fillContactNumber(value: string) {
    this.validationElements.contactNumberField().clear().type(value);
    return this;
  }

  fillDate(value: string) {
    this.validationElements.pickupDateField().clear().type(value);

    return this;
  }

  fillContactName(value: string) {
    this.validationElements.contactNameField().clear().type(value);
    return this;
  }
  ...
  verifyPageUrl() {
    cy.url().should(
      "eq",
      `${Cypress.config().baseUrl}${Cypress.env("form_validation_url")}`
    );
  }
