...
Given(
  "the user can see element {string} in the source container",
  verifyElement
);

Given(
  "the user can see element {string} in the target container",
  verifyElement
);

When(
  "the user drags and drops element {string} to the target drop zone of element {string}",
...
Then(
  "the target container should display {string} at column {string}",
  (element: string, column: string) => {
    dragAndDropPage.verifyDragAndDrop(element, column);
  }
);

Then(
  "the source container should display {string} at column {string}",
  (element: string, column: string) => {
    dragAndDropPage.verifyDragAndDrop(element, column);
  }
);
